...
Load the codebase-RAG skill via the `Skill` tool: `the `codebase-rag` skill`. The skill exposes `rag_search` (semantic search across the project) and `rag_query_impact` (semantic blast-radius for a candidate change). If the skill is not installed in the user's Claude Code session, the load will fail; this is one of the tool-failure stop conditions — halt with a card note and activity log entry naming the missing skill (`codebase-rag`) and stop.
...
2. `query-docs` — pass the library ID and a specific question (the behavior, API, or pattern you need to verify). Read the returned content.
