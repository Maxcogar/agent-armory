...
const PHASE_SCHEMA = {
  ...
  properties: {
    ...
    halt: {
      type: 'object',
      properties: {
        category: S_STR, // HARD-RULE-CONFLICT | PREMISE-FALSE | PLAN-FLAW | BLAST-RADIUS-EXCEEDS-PLAN | ENVIRONMENT-BLOCKED | SPEC-TRACEABLE | BUSINESS
        detail: S_STR,
...
const IMPLEMENT_SCHEMA = {
  ...
  properties: {
    ...
    stop_report: {
      type: 'object',
      properties: {
        category: { type: 'string', enum: ['HARD-RULE-CONFLICT', 'PREMISE-FALSE', 'PLAN-FLAW', 'BLAST-RADIUS-EXCEEDS-PLAN', 'ENVIRONMENT-BLOCKED'] },
        step: S_STR,
        asserted: S_STR,
        observed: S_STR,
        recommendation: S_STR,
...
if (cursor === 'architecture') {
  ...
  const gate = await runGate({
    ...
    remediateFn: (findings) => agent(`Correct the architecture at ${archPath} against these review findings. Re-derive each finding's section from that section's sources — do not edit the sentence the finding points at, and do not re-author the artifact; its untouched sections passed the prior round's review and are outside this correction's scope — a flaw you notice there is recorded in evidence (location, verified evidence, proposed correction), neither fixed nor dropped. Sweep each finding's class across the whole artifact. Correct only what the findings require. Return sections_rederived: one entry per re-derived section with its location (path:start-end or path#section), the source it was re-derived from, the finding it addresses, and class_sweep {searched, pattern, scope, found, sites_changed} — pattern is the executable regex the sweep ran (Grep syntax) and scope the file or glob it ran over (normally this artifact); found lists EVERY location the search returned, corrected or not; sites_changed lists the found locations you actually edited; every found entry outside sites_changed carries an open_sites entry {location, designation} naming its escalation or explicitly-open status. The pattern is re-executed independently against the artifact this same round: executing it over scope must reproduce found, and a found site neither changed nor designated fails the gate. A finding whose named standard you cannot verify, or that you have verified is wrong, returns status: 'halted' with the reason and evidence in halt.detail — it is not applied. Findings: ${JSON.stringify(findings)}`, { agentType: AGENT.corrector, schema: PHASE_SCHEMA, phase: 'Architecture', label: 'revise:arch' }),
    multiLens: false,
    detectFailedCorrection: true,
    sweepVerifyFn: (secs) => reExecuteSweeps(secs),
  })
...
if (cursor === 'plan') {
  ...
  const gate = await runGate({
    ...
    remediateFn: (findings) => agent(`Correct the plan at ${planPath} against these review findings. Re-derive each finding's section from that section's sources — do not edit the sentence the finding points at, and do not re-author the artifact; its untouched sections passed the prior round's review and are outside this correction's scope — a flaw you notice there is recorded in evidence (location, verified evidence, proposed correction), neither fixed nor dropped. Sweep each finding's class across the whole artifact. Correct only what the findings require. Return sections_rederived: one entry per re-derived section with its location (path:start-end or path#section), the source it was re-derived from, the finding it addresses, and class_sweep {searched, pattern, scope, found, sites_changed} — pattern is the executable regex the sweep ran (Grep syntax) and scope the file or glob it ran over (normally this artifact); found lists EVERY location the search returned, corrected or not; sites_changed lists the found locations you actually edited; every found entry outside sites_changed carries an open_sites entry {location, designation} naming its escalation or explicitly-open status. The pattern is re-executed independently against the artifact this same round: executing it over scope must reproduce found, and a found site neither changed nor designated fails the gate. A finding whose named standard you cannot verify, or that you have verified is wrong, returns status: 'halted' with the reason and evidence in halt.detail — it is not applied. Findings: ${JSON.stringify(findings)}`, { agentType: AGENT.corrector, schema: PHASE_SCHEMA, phase: 'Plan', label: 'revise:plan' }),
    multiLens: false,
    detectFailedCorrection: true,
    sweepVerifyFn: (secs) => reExecuteSweeps(secs),
  })
...
if (cursor === 'implement') {
  ...
  if (impl && impl.status === 'halted' && impl.stop_report) {
    ...
    // PREMISE-FALSE / PLAN-FLAW / BLAST-RADIUS -> auto amend-plan -> re-implement (remediation
