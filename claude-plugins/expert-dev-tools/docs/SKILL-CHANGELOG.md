...
One change, from the Context Oracle Phase A plan's correction loop
(`middleware/context-oracle/docs/collapse-log.md` 2026-09-07 and the round-2/round-3 review
records under `middleware/context-oracle/docs/reviews/`). The loop was stopped under the owner's
rule after two consecutive rounds in which the corrections themselves introduced defects; the
regression classes are general to any plan under this skill, so the fix lands here.

---

### 18 · `skills/expert-plan/references/output-contract.md` + `scripts/derive-plan-sections.mjs` + `scripts/run-plan-probes.mjs` (new) + `scripts/fixtures/` · build order, executed evidence, and the step→test table · **three drift classes removed from the authored surface**

**Trigger.** Round 3 of the Context Oracle Phase A plan review returned 43 findings, 23 tagged as
introduced by the round-2 corrections. Classified, they fall into three forms the contract left
hand-maintained: (1) a step consuming a module, CLI verb, or script that a later step creates —
the author reconciled identifiers, the reviewers walked the build in order, and every such
regression lay between those checks; (2) executed claims in section 11 transcribed from one-off
runs — a concurrency schedule asserted an outcome its own timeline could not produce, and no
reviewer could re-run a sentence; (3) the step→test table and each step's Dependencies line
restated by hand alongside the generated regions. Applied 2026-09-07.

**Grammar** (contract item 7; script; fixture; self-check):

- New required key `provides: [...]` — names, not paths, a step makes available to later steps
  (a CLI verb, a command, a script entry point), single tokens with `-` where the name has a space,
  `[]` when none.
- `depends_on` entries must be declared **earlier in the document**; a later dependency is an
  error (steps are ordered as written).
- **Build-order check.** A step's text (title line to the next title line, fenced code excluded)
  may name in backticks only artifacts that exist when it is built: a mention matching a path
  another step creates (equal, or a trailing segment sequence) or a name another step provides
  (first *k* words joined by `-`) must resolve to a declared or transitive dependency. A path
  created by two steps is an error.

**Evidence** (contract item 11; new script): an executed claim is kept as a **probe** — a script
in the plan's sibling `<plan-stem>.probes/` with its recorded stdout under `expected/`, cited as
`` `probe:<name>` ``. `derive-plan-sections.mjs --check` fails a citation with no probe, a probe
with no citation, and a probe with no expectation; `run-plan-probes.mjs <plan.md>` re-executes
every probe (optionally preparing a layout via `layout/prepare.sh`) and fails on drift.
`.optional.` probes may print `SKIPPED: <why>` when their environment is absent.

**Generated regions** (contract items 12 and the generated-regime section): section 12 opens
with a generated step→test table between `<!-- generated:tests begin/end -->` markers.

**Gate C** gains the probe run and the build-order property. Self-check grows to 27 checks: the
fixture now carries a provided name, a consumer naming its producer's artifacts, and a probe
citation resolved against `scripts/fixtures/valid-plan.probes/`; negative cases added for a
missing `provides` key, a later dependency, a duplicate creator, undeclared consumption of a
created path and of a provided name, a missing `tests` region, and a missing Plan heading; one
positive case asserts declared consumption passes. The plugin version is 0.4.2.

**Why here and not beside the plan.** A first version of this fix was written as project-local
Python beside the Context Oracle plan (a generator for its §5.1 tree and Dependencies lines, a
gate, a probe runner). The owner rejected it: tooling that works for one plan has no value. The
classes are properties of any plan under this skill; the skill is where they are enforced, and the
project copy of the skill is synced from here.
