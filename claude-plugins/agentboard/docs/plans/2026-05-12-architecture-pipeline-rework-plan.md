...
3. Process steps:
   - **Step 1**: cross-cutting expert-standards activation (verbatim text per Preamble rule 2).
   - **Step 2**: Bundle ingestion — read the verified bundle from the inline `arch_facts_bundle` passed by the orchestrator (the orchestrator resolves the verified bundle from the audit and passes its JSON body inline as `verified_bundle_json`; compose consumes it as `arch_facts_bundle`); same authority and field meanings as L3.
...
3. Process steps:
   - **Step 1**: cross-cutting expert-standards activation (verbatim text per Preamble rule 2).
   - **Step 2**: Bundle ingestion — read the verified bundle from the inline `arch_facts_bundle` passed by the orchestrator (the orchestrator resolves the verified bundle from the audit and passes its JSON body inline as `verified_bundle_json`; compose consumes it as `arch_facts_bundle`). At L1, design fields are likely minimal but present (e.g., few `files_relevant`, no `dependency_edges` or sparse, no `open_questions` typical).
   - **Step 3**: Read spec.
...
9. Verify the agent submitted an ARCH_BUNDLE_AUDIT_V2 artifact. Read verified_level (numeric) from it. Resolve the verified bundle into inline JSON for compose dispatch: when `any_discrepancy == true`, use the audit's `corrected_bundle`; when `any_discrepancy == false`, fetch the original ARCH_FACTS_BUNDLE_V2 body via `agentboard_get_workspace_artifact` on the audit's `audited_bundle_artifact_id` back-reference. Capture the result as `verified_bundle_json`. The current implementation passes this inline to compose at step 11 — see `commands/architecture.md` step 9 for the exact resolution sequence. (The auditor contract §1.2/§3 is unchanged.)
...
11. Dispatch:
    ...
    Pass to compose agent: spec_path, verified_level (numeric), scaffold_card_id, agent_id, and the verified bundle inline as `verified_bundle_json` (the orchestrator resolves the verified bundle from the audit and passes its JSON body inline; compose consumes it as `arch_facts_bundle`). On an architecture-document-route correction re-run also pass `correction_request_json` (the declared correction-loop input — short-spec CL-022..CL-029) and the prior architecture document's `prior_architecture_document_path` + `prior_architecture_document_artifact_id`. Wait for completion.
