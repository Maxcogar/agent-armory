...
- **Consumes:** `spec_path` (file path string), `scaffold_card_id`, `agent_id`, and an optional `force_remeasure` boolean (set by the orchestrator on a verified-bundle-route correction re-run per short-spec CL-009; see §9 step 17). When `force_remeasure == true`, the agent skips its prior-bundle-reuse check at Step 2 and re-measures into a fresh V2 bundle regardless of spec-hash match.
...
- **NOT in scope:** design reasoning, slice derivation, document authoring, soft-judgment overrides not anchored in a field discrepancy. The correction loop does not change this contract: a verified-bundle-route correction triggers a fresh independent audit of the freshly re-measured bundle; the auditor is never handed a correction as input (anchoring before independent measurement is the failure mode it exists to prevent — short-spec CL-009 routing and the existing auditor pipeline-boundary contract).

### 1.3 `architecture-compose-l3` (opus)

- **Consumes:** `spec_path`, `verified_level: 3`, `scaffold_card_id`, `agent_id`, and `audit_artifact_id` (the `ARCH_BUNDLE_AUDIT_V2` artifact ID — compose Step 2 fetches the audit via `agentboard_get_workspace_artifact` and resolves the verified `ARCH_FACTS_BUNDLE_V2` from it by the same `any_discrepancy`-branch logic `agents/architecture-design-reviewer.md` Step 2(c) uses; the orchestrator never embeds bundle JSON in the compose prompt). On a correction re-run also: `correction_request_json` (declared correction-loop input — short-spec CL-022..CL-029), and the prior architecture document's `prior_architecture_document_path` + `prior_architecture_document_artifact_id`.
...
- **Scope:** read inputs (Step 1 activation, Step 2 fetches the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` on `audit_artifact_id` and resolves the verified bundle by the `any_discrepancy` branch, binding it as `arch_facts_bundle`), goal, standards identification (including ISO 25010 mapping, ASVS when security in scope), spec problem detection, hard decisions in five-part format, threat model when security in scope, design decisions, quality characteristic mapping (10a), ASVS mapping (10b), document write (Phase 11), slicing (Phase 12). Clear Thought tools throughout per the 2026-05-09 plan's reasoning support table. Context7 `query-docs` against bundle library IDs (and `resolve-library-id` for libraries not in bundle). **Correction mode** (presence of `correction_request_json`): the prescribed correction-mode process per §6.3 — re-derive the targeted design decisions, structure, quality mappings, and slices the correction request actually reaches, preserve non-targeted material only when it still remains correct, re-run the whole document write + slice derivation + collaborativereasoning synthesis + gates + trap audit before submission, halt on underspecified correction. Read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback. No diff/preserve/patch of the prior document.
...
- **Consumes:** `spec_path`, `verified_level: 2`, `scaffold_card_id`, `agent_id`, and `audit_artifact_id` (the `ARCH_BUNDLE_AUDIT_V2` artifact ID — compose Step 2 fetches the audit via `agentboard_get_workspace_artifact` and resolves the verified `ARCH_FACTS_BUNDLE_V2` from it by the same `any_discrepancy`-branch logic `agents/architecture-design-reviewer.md` Step 2(c) uses; the orchestrator never embeds bundle JSON in the compose prompt). On a correction re-run also: `correction_request_json` (declared correction-loop input — short-spec CL-022..CL-029), and the prior architecture document's `prior_architecture_document_path` + `prior_architecture_document_artifact_id`.
- **Produces:** `architecture_document` artifact; `docs/arch/<file>.md`.
- **Scope:** read inputs (Step 1, Step 2 fetches the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` on `audit_artifact_id` and resolves the verified bundle by the `any_discrepancy` branch, binding it as `arch_facts_bundle`), goal, standards (lighter than L3), spec problems, design decisions in five-part format with inline verification approach per decision (replaces L3's Phase 10a/10b ceremony), two-pass write (Phase 7.5 body, Phase 8 slicing). No Clear Thought MCP invocations — disciplines stay inline per 2026-05-09 plan's §8.4. **Correction mode** (presence of `correction_request_json`): the prescribed correction-mode process per §6.4 — re-derive the targeted decisions, structure, traceability, and slices the correction request actually reaches, preserve non-targeted material only when it still remains correct, re-run the full document write + slice derivation + all gates before submission, halt on underspecified correction. Read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback. No diff/preserve/patch.
...
- **Consumes:** `spec_path`, `verified_level: 1`, `scaffold_card_id`, `agent_id`, and `audit_artifact_id` (the `ARCH_BUNDLE_AUDIT_V2` artifact ID — compose Step 2 fetches the audit via `agentboard_get_workspace_artifact` and resolves the verified `ARCH_FACTS_BUNDLE_V2` from it by the same `any_discrepancy`-branch logic `agents/architecture-design-reviewer.md` Step 2(c) uses; the orchestrator never embeds bundle JSON in the compose prompt). On a correction re-run also: `correction_request_json` (declared correction-loop input — short-spec CL-022..CL-029), and the prior architecture document's `prior_architecture_document_path` + `prior_architecture_document_artifact_id`.
- **Produces:** `architecture_document` artifact (slim — slices ARE the architecture); `docs/arch/<file>.md`.
- **Scope:** read inputs (Step 1, Step 2 fetches the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` on `audit_artifact_id` and resolves the verified bundle by the `any_discrepancy` branch, binding it as `arch_facts_bundle`), goal, standards (inherited from spec), slice the cards (1–3 cards by classification), single-pass write per 2026-05-09 plan's §8.5 Phase 6. **Correction mode** (presence of `correction_request_json`): the prescribed correction-mode process per §6.5 — re-derive only the architecture content the correction request actually targets (typically the affected slice(s) at L1), preserve non-targeted content only when it still remains correct, re-run the full document write + gate checks + trap audit for the whole document before submission, halt on underspecified correction. Read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback. No diff/preserve/patch.
...
- **Consumes:** `spec_path`, `architecture_document_path`, `architecture_document_artifact_id`, `audit_artifact_id`, `scaffold_card_id`, `agent_id`. (`audit_artifact_id` is the `ARCH_BUNDLE_AUDIT_V2` artifact id — the reviewer resolves the verified bundle from the audit itself; renamed from the prior misnamed `verified_bundle_artifact_id` as recorded in `docs/specs/spec-evidence.md` E-007/X-006 and the 2026-05-23 audit. Both path and artifact ID are provided for the architecture document — path for direct Read access, artifact ID for fetching via `get_workspace_artifact` if disk read fails.)
...
## 4.1 `correction_request_json` schema (correction loop declared input)

The correction loop's declared stage input to the affected compose agent. Governing design: `docs/specs/2026-05-16-correction-loop-option-a-design.md` (Phase-8 short spec, derived from `docs/specs/spec-ledger.yaml` CL-022..CL-029). This is NOT a submitted artifact — it is passed to compose in the prompt as an explicit, auditable declared input (short-spec CL-023, CL-028). The §7 validation hook does NOT gate this input; the four submitted architecture artifact types are the only types the hook recognizes.

```json
{
  "round": 1,
  "origin": "design-reviewer | owner-directed | source-traced-upstream-failure",
  "routed_target": "architecture-document | verified-bundle | spec",
  "requested_change": "<prose statement of the substantive correction to apply>",
  "provenance": "<finding id, user instruction, or failure-trace excerpt that justified the correction>"
}
```

- This shape is the implementer's concretization of the short spec's CL-023 (declared correction input) and CL-028 (correction work travels as a declared auditable input). The short spec does not mandate this exact name or shape; the runnable surfaces (`commands/architecture.md` step 17 and the three compose profiles' Correction-mode process sections) document the live schema.
- `routed_target` records the route selected by the orchestrator's real-time source-trace (short-spec CL-009). On the `architecture-document` route, compose receives this input plus `prior_architecture_document_path` and `prior_architecture_document_artifact_id`. On the `verified-bundle` route, the orchestrator re-runs the pipeline from research; compose does NOT receive the correction request. On the `spec` route, the orchestrator surfaces the issue to the external spec-modification path (short-spec CL-013/CL-014/CL-015); compose is not invoked at all.
- If the correction request cannot be resolved into a concrete architecture change from the declared inputs, the affected compose stage halts and surfaces the correction as underspecified rather than guessing or silently widening the requested change.
- A retry cap of 3 on the same scaffold card bounds the local correction loop (short-spec CL-018). At the cap, the orchestrator hands off to the external investigator path rather than dead-halting (short-spec CL-019). Escalation to the owner is limited to spec-origin outcomes (short-spec CL-020).
...
2. Process steps:
   ...
   - **Step 2**: Read spec at `spec_path` in full; perform the prior-bundle reuse check. When the orchestrator set `force_remeasure: true` (a verified-bundle-route correction re-run — short-spec CL-009), prior-bundle reuse is suppressed and the agent re-measures into a fresh V2 bundle regardless of spec-hash match.
...
4. Failure modes.
5. Correction loop: this profile is **unchanged** by the correction loop. A verified-bundle-route correction is handled as a fresh re-measure (orchestrator re-runs research with `force_remeasure: true`) followed by this auditor running normally; the auditor is never handed a correction as input — anchoring before independent measurement is the failure mode it exists to prevent (short-spec CL-009 routing and the existing auditor pipeline-boundary contract).
...
8. Process steps:
   - **Step 1**: cross-cutting expert-standards activation (verbatim text per Preamble rule 2).
   - **Step 2**: Fetch the `ARCH_BUNDLE_AUDIT_V2` artifact via `agentboard_get_workspace_artifact` on the orchestrator-passed `audit_artifact_id`, normalize the content (BOM/CR per the discipline `agents/architecture-design-reviewer.md` Step 2(c) uses), verify the `ARCH_BUNDLE_AUDIT_V2` sentinel, parse the JSON, and resolve the verified `ARCH_FACTS_BUNDLE_V2` by the audit's `any_discrepancy` branch: when `any_discrepancy == true`, read the audit's `corrected_bundle` object directly; when `any_discrepancy == false`, fetch the original bundle via `agentboard_get_workspace_artifact` on the audit's `audited_bundle_artifact_id` back-reference, normalize, strip the `ARCH_FACTS_BUNDLE_V2` sentinel, and parse. Bind the resolved bundle as `arch_facts_bundle` (the alias the rest of this profile uses). Halt with a scaffold-card note via `agentboard_update_workspace_card` and an activity log entry on sentinel mismatch, JSON parse failure, missing or non-object `corrected_bundle` when `any_discrepancy == true`, or transport failure on the original-bundle fetch — the orchestrator detects this halt via the absence of the compose artifact at `commands/architecture.md` step 12 and reads the failure card note. Read the resolved bundle as authoritative. Specifically: `files_relevant` is the file set the architecture addresses; `dependency_edges` describe coupling; `blast_radius` informs scope decisions; `existing_patterns_hits` informs pattern adherence/divergence decisions; `constraint_hits` are constraints the architecture must respect; `external_libraries` lists library IDs available for Context7 query-docs; `open_questions` enumerates ambiguities to resolve in design. Snippet existence is authoritative (auditor verified); snippet relevance to a specific decision is the agent's judgment.
   ...
   - **Correction mode** (entered iff `correction_request_json` is present — short-spec CL-022..CL-029; a prescribed process distinct from the create-from-scratch flow): (a) read `correction_request_json` as the authoritative statement of what must change in this round; read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback; (b) re-derive the targeted design decisions, structure, quality mappings, and slices the correction request actually reaches through the same five-part / Clear-Thought discipline (Steps 5–11); (c) preserve every non-targeted decision/section only when it still remains correct after the targeted re-derivation, by Reading the prior document — never diff/preserve/patched; (d) unconditionally re-run the traceability matrix, Steps 14–16 (collaborativereasoning, synthesis incorporation, Gates A/B/C + trap audit) and Step 13 Card-Slices re-derivation (slices derive from the changed document); (e) submit per Step 17. If the correction request cannot be resolved into a concrete architecture change from the declared inputs, halt and surface the correction as underspecified — do not guess or silently widen. No instruction in this profile may tell compose to diff, preserve, or patch the prior document (grep-checkable acceptance criterion).
...
3. Process steps:
   - **Step 1**: cross-cutting expert-standards activation (verbatim text per Preamble rule 2).
   - **Step 2**: Bundle ingestion — fetch the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` on the orchestrator-passed `audit_artifact_id`, resolve the verified `ARCH_FACTS_BUNDLE_V2` by the audit's `any_discrepancy` branch (mirroring §6.3 Step 2 above and `agents/architecture-design-reviewer.md` Step 2(c)), and bind the resolved bundle as `arch_facts_bundle`; same authority and field meanings as L3. Halt with a scaffold-card note via `agentboard_update_workspace_card` and an activity log entry on sentinel mismatch, JSON parse failure, missing or non-object `corrected_bundle` when `any_discrepancy == true`, or transport failure on the original-bundle fetch.
   ...
   - **Correction mode** (entered iff `correction_request_json` is present — short-spec CL-022..CL-029; a prescribed process distinct from the create-from-scratch flow): (a) read `correction_request_json` as the authoritative statement of what must change in this round; read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback; (b) re-derive the targeted decisions, structure, traceability, and slices the correction request actually reaches through Step 7's five-part discipline; (c) preserve non-targeted decisions only when they remain correct after the targeted re-derivation, by Reading the prior document — no diff/preserve/patch; (d) unconditionally re-run the traceability matrix, Step 10's inline Gates A/B/C + trap audit, and Step 9 Card-Slices re-derivation; (e) submit per Step 11. If the correction request cannot be resolved into a concrete architecture change from the declared inputs, halt and surface the correction as underspecified. No instruction may tell compose to diff, preserve, or patch the prior document (grep-checkable).
4. Failure modes.
...
3. Process steps:
   - **Step 1**: cross-cutting expert-standards activation (verbatim text per Preamble rule 2).
   - **Step 2**: Bundle ingestion — fetch the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` on the orchestrator-passed `audit_artifact_id`, resolve the verified `ARCH_FACTS_BUNDLE_V2` by the audit's `any_discrepancy` branch (mirroring §6.3 Step 2 above and `agents/architecture-design-reviewer.md` Step 2(c)), and bind the resolved bundle as `arch_facts_bundle`. At L1, design fields are likely minimal but present (e.g., few `files_relevant`, no `dependency_edges` or sparse, no `open_questions` typical). Halt with a scaffold-card note via `agentboard_update_workspace_card` and an activity log entry on any resolution failure (sentinel mismatch, JSON parse failure, missing or non-object `corrected_bundle` when `any_discrepancy == true`, or transport failure on the original-bundle fetch).
   - **Step 3**: Read spec.
   ...
   - **Correction mode** (entered iff `correction_request_json` is present — short-spec CL-022..CL-029; a prescribed process distinct from the create-from-scratch flow): at L1 the slicing IS the architecture, so the correction request typically targets the affected slice(s). (a) Read `correction_request_json` as the authoritative statement of what must change in this round; read the prior architecture document from `prior_architecture_document_path` with `prior_architecture_document_artifact_id` fallback; (b) re-derive only the architecture content the correction request actually targets (typically the affected slice(s)) per Step 6; (c) preserve every non-targeted slice only when it remains correct after the targeted re-derivation, by Reading the prior document — no diff/preserve/patch; (d) unconditionally re-run Step 8's single delivery gate, Step 9's trap audit, and slice-consistency; (e) submit per Step 10. If the correction request cannot be resolved into a concrete architecture change from the declared inputs, halt and surface the correction as underspecified. No instruction may tell compose to diff, preserve, or patch the prior document (grep-checkable).
4. Failure modes.
...
**`ARCH_DESIGN_REVIEW_V1` rule set (4 rules):**
...
R-REVIEW-4: .audit_artifact_id is present and is a non-empty string (the design-reviewer seam — short-spec correction-path consistency; the reviewer resolves the verified bundle from this audit artifact id per the §1.6 boundary contract).
```

**Synthetic test fixtures** for the rework's Session-N test step (per the plan author discipline acceptance criterion): one valid + one invalid synthetic for each of the four submitted artifact types; valid architecture document tested at each level (L1, L2, L3). The correction-loop input (`correction_request_json`) is a declared stage input, not a submitted artifact, and is not gated by this hook — see §4.1 for that input's schema.
...
4. Select or create a workspace board via agentboard MCP. (The `/architecture` correction pause is opt-in via a declared `--pause` / `pause` flag in the command argument string, default off — short-spec CL-010; this work introduces no AgentBoard app change and does not depend on the app's blocking / auto_transitions mechanism.)
...
9. Verify the agent submitted an ARCH_BUNDLE_AUDIT_V2 artifact and capture its artifact ID as `audit_artifact_id`. Fetch the audit content via `agentboard_get_workspace_artifact` on that ID and read `verified_level` (numeric integer in `{1, 2, 3}`) for compose dispatch in step 11. The orchestrator does NOT resolve the verified bundle from the audit — that resolution moves to compose's Step 2 (mirroring how `agents/architecture-design-reviewer.md` Step 2(c) already does it). The orchestrator threads `audit_artifact_id` forward unchanged: passed to compose at step 11 AND to the design reviewer at step 14, the same single value. This preserves single-source-of-truth (the audit is the only place the verified bundle lives), honest-interface (`audit_artifact_id` points to a real artifact a real agent produced), and removes the orphan-artifact failure mode the old "materialize a verified-bundle artifact" approach introduced. (The auditor contract §1.2/§3 is unchanged.)
...
11. Dispatch:
    ...
    Pass to compose agent: spec_path, verified_level (numeric), scaffold_card_id, agent_id, and `audit_artifact_id` (the same `ARCH_BUNDLE_AUDIT_V2` artifact ID also passed to the design reviewer at step 14). Compose Step 2 fetches the audit and resolves the verified bundle by the same `any_discrepancy`-branch logic the design reviewer uses. Never pass bundle JSON inline. On an architecture-document-route correction re-run also pass `correction_request_json` (the declared correction-loop input — short-spec CL-022..CL-029) and the prior architecture document's `prior_architecture_document_path` + `prior_architecture_document_artifact_id`. Wait for completion.
...
14. Spawn architecture-design-reviewer (background). Pass spec_path, architecture_document_path, architecture_document_artifact_id, audit_artifact_id (the ARCH_BUNDLE_AUDIT_V2 id — the reviewer resolves the verified bundle from the audit itself; renamed from the prior misnamed verified_bundle_artifact_id as recorded in `docs/specs/spec-evidence.md` E-007/X-006), scaffold_card_id, agent_id. Wait for completion.
...
17. Correction loop (short-spec CL-008..CL-029; supersedes the prior re-spawn-with-context workaround that the FAILED handoff flagged as the §9-step-17 contradiction). On a correction (a design-review finding, an owner-directed change, or a failure source-traced back to architecture): determine the route by real-time source-trace among {architecture-document, verified-bundle, spec}; construct a declared `correction_request_json` input (§4.1 schema) with `round` (incremented round counter), `origin` ∈ {`design-reviewer`, `owner-directed`, `source-traced-upstream-failure`}, `routed_target` ∈ {`architecture-document`, `verified-bundle`, `spec`}, non-empty `requested_change`, and non-empty `provenance`; then by route: **architecture-document** ⇒ re-enter the level-appropriate compose stage in correction mode with `correction_request_json` + `prior_architecture_document_path` + `prior_architecture_document_artifact_id`, then repeat steps 12 through 16 on the same scaffold card; **verified-bundle** ⇒ re-run the pipeline from the research wave (repeat steps 6–16 on the same scaffold card), with no correction input fed to research or the auditor; **spec** ⇒ hand off to the external spec-modification path rather than `/foundation` or an in-flow `spec_path` edit (short-spec CL-013/CL-014/CL-015), and stop the local correction loop. The loop has a finite retry cap of 3 on the same scaffold card (short-spec CL-018); at the cap, hand off to the external investigator path and stop the local correction loop rather than dead-halting (short-spec CL-019). The investigator handoff is automatic at the cap and is separate from the design reviewer; `/architecture` records the handoff via a scaffold-card note and activity log entry naming the round cap, the current routed evidence, and that an external investigator handoff is now required. Escalate to the owner only when the external source-trace process determines the issue is spec-origin (short-spec CL-020). The `/architecture` correction pause is opt-in and off by default (short-spec CL-010): when `architecture_correction_pause == true`, pause before applying a routed substantive correction; otherwise the loop runs autonomously. **No silent or automatic `Edit` of `spec_path` anywhere** (short-spec CL-012). Minor corrections (wording, traceability rows, typos, non-substantive single-sentence rewordings) are applied via `Edit` directly on the file at `architecture_document_path`; only those bypass the correction-loop re-derive, and the user re-approves before commit.
...
9. **Validation hook exists and behaves.** Hook script exists at `hooks/scripts/validate-architecture-artifact.sh`, registered in `hooks/hooks.json` as a PreToolUse hook on `mcp__agentboard__agentboard_submit_workspace_artifact`. Fires for the four submitted architecture-pipeline artifact types (`architecture_document`, `ARCH_FACTS_BUNDLE_V2`, `ARCH_BUNDLE_AUDIT_V2`, `ARCH_DESIGN_REVIEW_V1`) via content-type dispatch. Performs structural-only checks per type-specific rule sets per §7. Blocks invalid submissions with structured error to stderr + non-zero exit; passes valid submissions with exit 0. The correction-loop input (`correction_request_json`) is a declared stage input, not a submitted artifact, and is not gated by this hook.

10. **Hook synthetic-artifact tests pass.** Test suite contains invalid + valid synthetic for each of the four submitted artifact types; valid architecture document at each level (L1, L2, L3). All tests pass — invalid blocked with structured error, valid passed.
...
21. **Correction loop conforms to the approved short spec.** The implemented correction loop satisfies the ledger items CL-001..CL-029 of `docs/specs/2026-05-16-correction-loop-option-a-design.md` (Phase-8 short spec, derived from `docs/specs/spec-ledger.yaml`): the design reviewer's input is `audit_artifact_id` with the apology sentence gone, and the rename is propagated to `docs/specs/2026-05-12-agentboard-app-arch-pipeline-support.md`, `docs/handoffs/2026-05-13-session-6-to-7.md`, `hooks/tests/build-fixtures.py`, and `hooks/tests/fixtures/review_*.json`; no step in `commands/architecture.md` performs a silent or automatic `Edit` of `spec_path` (CL-012); each of the three compose profiles declares `correction_request_json`, `prior_architecture_document_path`, and `prior_architecture_document_artifact_id` as correction-loop inputs and carries a distinct correction-mode process (targeted re-derivation, prior document read with artifact-id fallback, whole-document write + gates + trap audit re-run, halt on underspecified correction; zero diff/preserve/patch instructions, grep-checkable) per CL-023..CL-028; `commands/architecture.md` step 17 runs real-time source-trace routing across three origins (`architecture-document`, `verified-bundle`, `spec`) with no static table per CL-008/CL-009; enforces a finite retry cap of 3 on the same scaffold card per CL-018; hands off to an external investigator at the cap per CL-019; escalates to the owner only on spec-origin outcomes per CL-020; spec-origin handoff goes to the external spec-modification path, not `/foundation` or an in-flow edit (CL-013/CL-014/CL-015); the `/architecture` correction pause is opt-in (off by default) via the `--pause` / `pause` flag in the argument string and is distinct from AgentBoard board blocking gates (CL-010); the §7 validation hook recognizes only the four submitted artifact types — the correction-loop input is a declared stage input, not a submitted artifact (Codex-remediation-audit Open Issue #5 from `spec-session-status.md`); an unresolved correction is surfaced as underspecified, never guessed (CL-027). Additionally, the bundle-passing seam between orchestrator and compose follows the same artifact-ID discipline as the design reviewer: each of the three compose profiles declares `audit_artifact_id` as a non-correction declared input and its Step 2 fetches the `ARCH_BUNDLE_AUDIT_V2` via `agentboard_get_workspace_artifact` and resolves the verified `ARCH_FACTS_BUNDLE_V2` from it by the audit's `any_discrepancy` branch (mirroring `agents/architecture-design-reviewer.md` Step 2(c)); `commands/architecture.md` step 11 passes only `audit_artifact_id` to compose (the same value passed to the design reviewer at step 14) and never embeds bundle JSON in any prompt; the orchestrator never parses the bundle. This is an engineering-standards conformance criterion (single source of truth, honest interface, no orphan artifacts), not a short-spec CL-* mandate — the short spec is silent on the bundle-passing seam. Verified by grep + tracing `commands/architecture.md` steps against the short spec and the compose profiles' Correction-mode process sections, AND by grep-confirming zero `verified_bundle_json` references in any runnable or governing surface.
