...
- Process step 1 is the cross-cutting expert-standards activation. Process step 2 fetches the verified `ARCH_FACTS_BUNDLE_V2` from the audit referenced by the orchestrator-passed `audit_artifact_id`: compose calls `agentboard_get_workspace_artifact` on `audit_artifact_id`, parses the `ARCH_BUNDLE_AUDIT_V2` JSON, branches on the audit's `any_discrepancy` flag, and either reads `corrected_bundle` directly (when `any_discrepancy == true`) or fetches the original bundle via the audit's `audited_bundle_artifact_id` back-reference and parses it (when `any_discrepancy == false`) — the same resolution logic `agents/architecture-design-reviewer.md` Step 2(c) uses. Treat snippet existence (RAG hits, dependency edges, file lists with roles) as authoritative ground truth; treat RAG hit relevance to a specific decision as advisory. Subsequent process reads from bundle fields. The orchestrator passes only `audit_artifact_id` (the same value also passed to the design reviewer at step 14) and never embeds bundle JSON in the compose prompt — this preserves the single-source-of-truth and honest-interface properties the rework was derived against (the audit is the only place the verified bundle lives; no orchestrator-side serialization).
