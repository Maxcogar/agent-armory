...
description: Phase B review of the architecture pipeline. Read the architecture document, spec, and verified bundle (derived from an ARCH_BUNDLE_AUDIT_V2 audit artifact); surface design defects in six named categories — missing decisions, unjustified slices, contract mismatches between produces/consumes pairs, standards-decoration, decision-hiding, deferred-decision — plus an `other` catch-all for defects that fit none of the six. Emit one ARCH_DESIGN_REVIEW_V1 artifact with severity-tagged findings; the list may be empty. Advisory only — do not block approval. Invoke from /architecture after the validation hook passes compose's artifact and before user approval.
...
**(a) Extract spec R#/Q# identifiers.** `Grep` `spec_path` with `\bR[0-9]+\b` and `\bQ[0-9]+\b` (adjust to the spec's labeling convention if it differs — e.g., `R-1`, `R.1`). Sanity-check the matched set against the spec's requirements section: locate the requirements section heading via case-insensitive `Grep` on `spec_path` with prefix-anchored patterns (no `$` anchor, so each pattern matches the beginning of a heading line — variants like `## Requirements (functional)` or `## Acceptance criteria` are matched by the prefix) — in this priority order: (1) `^## Requirements`, (2) `^## Functional requirements`, (3) `^## Acceptance criteria`, (4) `^## Quality requirements`, (5) `^## Functional` (broader fallback), (6) `^## Quality` (broader fallback). Walk the list in order and stop at the first pattern that matches at least one heading; if multiple headings in the spec match that first-matching pattern, treat the union of their section bodies (from each matched heading until the next `##` heading) as the requirements-section body. From `spec_requirements`, discard any identifier that does not appear within that body — this filters false positives such as `R5xx`, `Q3 roadmap`, or `R2 compliance` mentions in unrelated prose. If none of the six patterns matches any heading, log via `agentboard_add_log_entry` a non-halting note naming Step 3(a) and the no-requirements-section-located condition, then hold `spec_requirements` as the full unfiltered matched-identifier set (filtering is silently a no-op only when the requirements section cannot be located; never when it could be). Hold the deduplicated set as `spec_requirements`.
...
**(a) Re-read the architecture document focused on slice ownership, boundary clarity, and cross-section consistency.** Look for at minimum: a slice whose Description does not match its Allowed-touch (e.g., the Description names a capability the Allowed-touch files do not implement); two decisions in `## Design decisions` that contradict each other (one says X, another says not-X); a `Components and structure` section (at L2/L3) describing a component not present in any slice's Allowed-touch; a `Limitations` entry that names a defect the document does not address; a `## Traceability matrix` row that references a `D#` not present in `## Design decisions`; a slice `Depends on` field naming a slice title that does not appear elsewhere in `## Card Slices`. This list is illustrative, not exhaustive — apply the expert-standards skill's judgment to flag any other cross-section inconsistency, cross-reference integrity failure, or structural incoherence not captured by the named patterns above.
...
- `findings` IDs are contiguous and ascending: when `findings` is non-empty, the sorted list of integer suffixes parsed from each `id` (the digits after the `F` prefix) equals `[1, 2, ..., len(findings)]`. No gaps, no duplicates, no out-of-order entries.
...
Submit via `agentboard_submit_workspace_artifact` with `type: "general"`, `scaffold_card_id`, `agent_id`, and content equal to the sentinel line `ARCH_DESIGN_REVIEW_V1` followed by the JSON. Log via `agentboard_add_log_entry` naming the submitted artifact ID returned by `agentboard_submit_workspace_artifact`, `document_level`, the length of `findings` (computed from the array, not from a top-level field — the schema has no `total_count`), and the per-severity counts (`summary.blocker_count`, `summary.serious_count`, `summary.minor_count`).
...
Use this section to find the halt behavior for a given failure condition. The inline halt instruction in each Process step is authoritative when wording differs from an entry below.
