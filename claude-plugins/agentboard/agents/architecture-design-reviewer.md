...
description: Phase B review of the architecture pipeline. Reads the architecture document, spec, and verified bundle (derived from an ARCH_BUNDLE_AUDIT_V2 audit artifact); surfaces design defects in six named categories — missing decisions, unjustified slices, contract mismatches between produces/consumes pairs, standards-decoration, decision-hiding, deferred-decision — plus an `other` catch-all for defects that fit none of the six. Emits one ARCH_DESIGN_REVIEW_V1 artifact with severity-tagged findings; the list may be empty. Advisory only — does not block approval. Invoke from /architecture after the validation hook passes compose's artifact and the auditor has produced the ARCH_BUNDLE_AUDIT_V2 artifact, and before user approval.
...
- The required sections for the parsed `document_level` appear in the document in their required relative order. The check is a subsequence check: each required heading must appear at a document position later than the prior required heading; optional sections (any `##` heading not in the required list) may appear between required headings without failing the check. Matching rule: each required heading entry below is matched as a prefix against the document's `##`-level headings — a document heading satisfies an entry when the heading line begins with the entry's text (so the document heading `## Goal — what this architecture serves` satisfies the entry `## Goal`). Only the `^## Card Slices$` Grep above is anchored exactly; the subsequence-list entries below are prefix matches. Required sections by level (in order):
...
If any of the five conditions fails, halt and log naming Step 2, the specific failing condition (heading missing / `## Card Slices` missing / level marker missing or malformed or out-of-range / required-section subsequence broken with the specific section name / bundle top-level field missing with the specific field name), and `document_level` (or "unparseable" if the level marker failed). If the failing condition is one the validation hook (per plan §7 R-DOC-2) was designed to catch — the `# Architecture — ` heading, `## Card Slices`, or the `**Level:** L#` marker — state in the log that the validation hook should have blocked this input upstream and recommend re-running compose against the same audit. For the other conditions (required-section subsequence break under prefix matching, or bundle top-level field missing), state in the log that the reviewer applies stricter or distinct checks than the hook; the upstream hook may have passed the input correctly and the appropriate user action is to inspect the document/bundle for the specific structural problem named. No `ARCH_DESIGN_REVIEW_V1` artifact has been submitted at this halt point; there is no artifact state to clean up. Stop.
...
**(a) Extract spec R#/Q# identifiers.** `Grep` `spec_path` with `\bR[0-9]+\b` and `\bQ[0-9]+\b` (adjust to the spec's labeling convention if it differs — e.g., `R-1`, `R.1`). Sanity-check the matched set against the spec's requirements-bearing sections: locate matching section headings via case-insensitive `Grep` on `spec_path` with prefix-anchored patterns (no `$` anchor, so each pattern matches the beginning of a heading line — variants like `## Requirements (functional)` or `## Acceptance criteria` are matched by the prefix). Collect the union of all matched section bodies (from each matched heading until the next `##` heading) into a single `requirements_scope`. The patterns to apply (all of them, not stop-at-first — a spec with both `## Functional requirements` and `## Acceptance criteria` contributes both section bodies to `requirements_scope`):
- `^## Requirements`
...
If at least one pattern matched a heading: from `spec_requirements`, discard any identifier that does not appear within `requirements_scope` — this filters false positives such as `R5xx`, `Q3 roadmap`, or `R2 compliance` mentions in unrelated prose. If none of the patterns matches any heading, log via `agentboard_add_log_entry` a non-halting note naming Step 3(a) and the no-requirements-section-located condition, then hold `spec_requirements` as the full unfiltered matched-identifier set (filtering is silently a no-op only when no requirements-bearing section can be located; never when one or more can be). Hold the deduplicated set as `spec_requirements`.

**(b) Locate the spec's stated Goal.** `Grep` `spec_path` for `^## Goal` or `^# Goal` (case-insensitive, prefix match — no `$` anchor, so heading variants such as `## Goal — what problem this solves` also match). The matched section's body content (from the matched heading line until the next `##`/`#` heading) is the stated Goal. If no match is found, the stated Goal is empty and the Goal-severity trigger in step (d) below does not fire for any requirement.
...
- `details`: brief reasoning naming which of conditions (i), (ii), or (iii) from step (c) failed: file absent from `files_relevant` (bundle-coverage gap on (i)); file present in `files_relevant` but with role `dependency` (bundle-coverage gap on (ii) — the slice may need the file classified as `candidate-modified`); slice has no source attributions at all (slicing gap on (iii) — the slice's `effective_requirements_set` is empty, which fails (iii) for every file in the slice's Allowed-touch uniformly). When the failure is (i) or a dependency-role (ii), the `details` must also name the bundle-coverage gap explicitly and state that the user should route the question to re-audit so the bundle can be corrected (the bundle may have missed the file or classified it too narrowly).
...
**(a) Parse the Standards table.** Each row names a standard, its source (file path or publication identifier), and its scope of governance (the third column of the Standards table, headed `what the standard governed` in the compose templates).
...
The rationale above governs ORDERING only — which category wins when a defect fits two or more. It does NOT govern MEMBERSHIP — whether a defect fits a category in the first place. For membership, apply each Step's definition literally: a defect fits `contract-mismatch` only when it matches Step 5's orphan-or-annotation-mismatch test against Produces/Consumes fields; a defect fits `missing-decision` only when it matches Step 3's spec-R#/Q#-not-in-addressed-set test; and so on. Do not use the blocking-severity rationale to coerce a defect into the highest-priority category — that test only fires when the defect already legitimately fits two or more category definitions and the question is which to prefer.

If a defect fits two or more named categories under reasonable readings of each Step's literal definition, use the higher-priority category and note the secondary category in `details`.
...
- `findings` IDs are contiguous and ascending: when `findings` is non-empty, for every index `i` in `[0, len(findings) - 1]`, `findings[i].id` equals `"F" + str(i + 1)`. That is, the first finding has `id == "F1"`, the second has `id == "F2"`, and so on through `id == "F<len(findings)>"` for the last entry. No gaps, no duplicates, no out-of-order entries — this is a positional check on the array, not a set-equality check on the suffix set.
...
When a halt condition fires, follow the inline halt instruction in the Process step where the condition was named — that text is authoritative. The entries below restate each halt path with the matching condition for cross-reference; when the wording in an entry below differs from the inline Process step's instruction, follow the Process step.
...
- **Audit artifact retrieval fails, audit JSON parse fails, `audit.schema_version != "2.0"`, `audit.rules_version != "1.0"`, `audit.verified_level` is missing or non-integer, or `audit.any_discrepancy` is missing or not a JSON boolean (true / false).** Halt at Step 2(c). Log naming Step 2, the failing input (audit), and the specific failure with values found and expected.
- **When `any_discrepancy == true`: `audit.corrected_bundle` is `null`, absent, or present but not a JSON object (e.g., it is a string, array, number, or boolean).** Halt at Step 2(c). Log naming Step 2, the failing input (audit), and the specific failure (absent-`corrected_bundle`-with-`any_discrepancy: true`, or `corrected_bundle` present but not a JSON object — name which).
...
- **Architecture document or resolved bundle is structurally malformed:** the `# Architecture — ` heading is absent, `## Card Slices` is absent, the `**Level:** L#` marker is absent or malformed or out-of-range, the required-section subsequence for `document_level` is broken (per prefix-match against the required-heading list in Step 2(d)), or the resolved bundle is missing a required top-level field (`schema_version`, `rules_version`, `spec_path`, `spec_hash`, `classification_fields`, `design_fields`, `rule_evaluation`, `agent_metadata`). Halt at Step 2(d). Log naming Step 2, the specific malformedness (which condition, with the specific section or field name), and `document_level` (or "unparseable" if the level marker check itself failed). For hook-covered conditions (the `# Architecture — ` heading, `## Card Slices`, the `**Level:** L#` marker), state in the log that the validation hook should have blocked this upstream. For non-hook-covered conditions (subsequence break under prefix matching, bundle top-level field missing), state in the log that the reviewer applies stricter or distinct checks than the hook. Do not record the malformedness as a finding.
