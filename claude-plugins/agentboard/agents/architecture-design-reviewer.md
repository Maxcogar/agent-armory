...
description: Phase B review of the architecture pipeline. Reads the architecture document, spec, and verified bundle; surfaces defects in the design — missing decisions, unjustified slices, contract mismatches between produces/consumes pairs, standards-decoration, decision-hiding, deferred-decision — plus an `other` catch-all for defects that fit none of the six named categories. Emits one ARCH_DESIGN_REVIEW_V1 artifact with severity-tagged findings; the list may be empty. Advisory; user sees findings alongside document and decides. Runs after the validation hook passes compose's artifact and before user approval.
...
- **You consume:** `spec_path`, `architecture_document_path`, `architecture_document_artifact_id`, `audit_artifact_id`, `scaffold_card_id`, `agent_id`.
...
- `document_level == 2`: a requirement is addressed when at least one decision in `## Design decisions` names it (compose-l2 records R#/Q# attributions as part of the decision's body content, not under a separate "Requirements addressed" subsection — per compose-l2's line 230: "For each design decision, record the spec requirements (R# and/or Q#) it addresses"), OR when it appears in `## Traceability matrix`, OR when it appears in the `**Out of scope:**` content of `## Scope (in / deferred / out)`. Locate the L2 Scope section via `Grep` for `^## Scope \(in / deferred / out\)` (start of line, escaped parens), then read the `**Out of scope:**` bold-labeled paragraph within. Content within the `**Deferred:**` paragraph of the same section is NOT counted as addressed. The slice-level `Source decisions` field at L2 names `D#` references — trace each `D#` to the D# entry in `## Design decisions` and read the entry's body for any R#/Q# attributions to enrich the addressed-set.
- `document_level == 3`: a requirement is addressed when at least one decision in `## Design decisions` names it (same body-content recording as L2 — compose-l3 line 316), OR when it appears in `## Traceability matrix`, OR when it appears in the `**Out of scope:**` content of `## Scope` (note: L3 uses plain `## Scope` per compose-l3 line 303-304, NOT `## Scope (in / deferred / out)` — locate via `Grep` for `^## Scope\r?$` or `^## Scope` followed by the delimited-prefix delimiter rule, then read the `**Out of scope:**` bold-labeled paragraph within). Content within `**Deferred:**` is NOT counted as addressed. Additionally `## Quality characteristics addressed (ISO/IEC 25010:2023)` may enumerate Q# attributions tying each Q# to a quality characteristic and decision; include these Q# references in the addressed-set.
...
From each slice's `Source decisions`, extract R#/Q# attributions using the same R#/Q# pattern that Step 3(a) used (the default `\bR[0-9]+\b` and `\bQ[0-9]+\b`, or the adjusted pattern Step 3(a) selected to match a non-default spec labeling convention like `R-1` or `R.1`). Extract D# references via `Grep` with `\bD[0-9]+\b`. The same code-block exclusion algorithm from Step 3(a) applies — but `Source decisions` field content is unlikely to contain fenced or indented code blocks in practice.

**(b) Resolve D# references at L2/L3.** For each D# referenced in a slice's Source decisions, locate the D# entry in `## Design decisions` via `Grep` for the D# identifier as a heading or bold label, then read the entry's full body (the five-part decision format per compose-l2/l3 — element 1 decision, element 2 standard, element 3 why standard applies, element 4 what NOT and why, element 5 premise verification + per-decision verification approach). Extract R#/Q# attributions from the entry's body via the same R#/Q# pattern used in Step 3(a) (including any labeling-convention adjustments applied there — see step (c) note below on format consistency). At L3, also check whether the D# is referenced in `## Quality characteristics addressed (ISO/IEC 25010:2023)`; if so, add any Q# attributions tied to it there. The union of direct R#/Q# attributions and D#-resolved R#/Q# attributions is the slice's `effective_requirements_set`.

Format-consistency note: if Step 3(a) adjusted the R#/Q# extraction pattern to match a non-default spec labeling convention (e.g., `R-1`, `R.1`), apply the same adjusted pattern here in Step 4(a) and 4(b). Hardcoding `\bR[0-9]+\b` / `\bQ[0-9]+\b` when Step 3(a) used a different pattern would discover the requirements at Step 3 but then fail to recognize them in slice attributions at Step 4. Carry the pattern forward.
...
Build, across all slices in `slices` (iterate in document order — the map preserves it from Step 4(a)): the set of slices that produce each contract, the set of slices that consume each contract, and the annotations attached to each entry. Compose-l2/l3 emit Produces and Consumes entries in the format `<contract name or interface> — consumed by <card title(s)>` and the mirror form `<contract name or interface> — produced by <card title(s)>` (em-dash, NOT parentheses — per compose-l2 lines 281-282 and compose-l3 lines 356-357). Parse each Produces entry by splitting on ` — consumed by ` (space-em-dash-space-`consumed by`-space): the substring before the separator is the contract name; the substring after is one or more card titles, comma-separated when multiple. Parse Consumes entries by splitting on ` — produced by `. Slice-name matching for the annotation check is exact case-sensitive string match after stripping the following from both the annotation text and the consumer/producer slice titles: leading and trailing whitespace; markdown emphasis markers (`**`, `__`, `*`, `_`); and trailing punctuation characters (`.`, `,`, `;`, `:`, `!`, `?`) — compose may emit annotations as sentence-final text, producing `— consumed by AuthCard.` for a slice titled `AuthCard`, and the trailing-period stripping prevents false annotation-mismatch findings from that purely stylistic difference. When multiple card titles are listed in one annotation (comma-separated), split on `,` (with surrounding whitespace stripped) and treat each as a separate consumer or producer membership. Substring matching is not used; fuzzy matching is not used; case-insensitive matching is not used. A contract is paired when both its producer set and consumer set are non-empty AND the annotations on each side name slices in the corresponding set. Two failure modes:
...
**(a) Parse the Standards table.** Each row names a standard, its source (file path or publication identifier), and its scope of governance (the third column of the Standards table, headed `what the standard governed` in the compose templates). The standard name is the verbatim text of the first non-empty cell of each table row (the leftmost column) after stripping leading/trailing whitespace and markdown emphasis markers (`**`, `__`, `*`, `_`). Use this string as a LITERAL-text match (fixed-string Grep) in step (b) below — NOT as a regex pattern. Standard names commonly contain regex metacharacters: parentheses in `OWASP ASVS 4.0 (Authentication)`, dots in `ISO/IEC 27001:2022`, brackets in `[RFC 9457]`, plus signs in `C++17`, asterisks in `*nix` POSIX references. A regex pattern built from raw text would either over-match (the `.` matching any character widens the search) or fail entirely (an unescaped `(` is a group-opening operator). Use the `Grep` tool's literal-string mode (e.g., `-F` for fixed-string semantics when the underlying tool supports it; otherwise escape every regex metacharacter — `\`, `^`, `$`, `.`, `|`, `?`, `*`, `+`, `(`, `)`, `[`, `]`, `{`, `}` — by prepending `\` before each occurrence in the standard name before using it as a Grep pattern).

**(b) Verify governance.** A standard is governing when one of the following mechanical conditions holds:

- The standard name appears in a `D#`'s body in `## Design decisions` (at L2/L3) under the five-part decision format — specifically in element 2 (the authoritative standard or first-principles anchor) or element 5 (premise verification + per-decision verification approach): `Grep` the `## Design decisions` section for the standard name as a literal-string match per step (a)'s escaping discipline. If it appears within a `D#` entry, the standard governs that D#.
- The standard name appears in a slice's Description or Source decisions content under `## Card Slices` (at any level): `Grep` the `## Card Slices` section for the standard name as a literal-string match per step (a)'s escaping discipline; if it appears within a slice's Description or Source decisions field, the standard governs that slice.
- The standard is inherited from the spec AND the spec's standards section names a realization the architecture is honoring: locate the spec's standards section via case-insensitive `Grep` on `spec_path` for `^## Standards that govern this spec` or `^## Standards`; then search within that section for the standard name as a literal-string match per step (a)'s escaping discipline. If the standard name appears, AND the architecture document's Standards row names the same inheritance in its governance column, the inheritance is the realization — no finding.
...
- `document_level == 2` or `3`: every `D#` entry in `## Design decisions` is treated as non-trivial. The reasoning: any decision worth committing to a `D#` heading is by definition a chosen path among at least implicit alternatives. Compose-l2/l3's five-part decision format (per compose-l2 line 211 and compose-l3 line 316) explicitly requires naming alternatives in element 4 ("what this decision is NOT — and why"). Treating every `D#` as non-trivial removes the membership ambiguity that a subjective "names a choice between two or more options" test would create — two independent reviewer runs on the same document identify the same non-trivial-decision set.
...
- At L2/L3: the `D#`'s body contains the five-part decision format's element 4 — the "what this decision is NOT and why" content naming at least one rejected alternative with the reason it is wrong. The five parts per compose-l2/l3 are: (1) decision, (2) standard or first-principles anchor, (3) why the standard applies, (4) what this is NOT and why, (5) premise verification + per-decision verification approach. Element 4 is the alternative-naming slot; a `D#` entry whose body has elements 1, 2, 3, 5 but lacks element 4 (no alternatives named, or alternatives mentioned without the "why each is wrong" reasoning) is decision-hiding. Detect element 4 presence by scanning the `D#` entry body for either an explicit element-4 marker (e.g., "What this is NOT", "Alternatives considered", "Rejected:", "Instead of"), or a paragraph that contrasts the chosen decision against named alternatives with reasoning for each rejection.
