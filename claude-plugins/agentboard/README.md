...
|---|---|---|
| **Best for** | A new project or a substantial change with real architectural risk — anything you want documented before code is written | Cleanup sweeps and lists of independent tasks (via `/sweep`); or work that needs an architecture pass but not the full 13-phase project ceremony (via `/foundation` → `/architecture`, which runs a level-aware pipeline that produces L1, L2, or L3 architecture rigor based on the spec's measured shape) |
...
Reads an approved spec from `/foundation` and runs the level-aware architecture pipeline that produces an architecture document at `docs/arch/<topic>.md` plus one workspace card per Card Slice in that document. The pipeline has three phases: (1) a haiku **research agent** measures eight countable fields from the spec and codebase and applies v1.0 classification rules to compute an L1 / L2 / L3 level; (2) a haiku **classification auditor** independently re-measures every field before looking at the research agent's output (anchoring-bias discipline) and emits a verified level (with a corrected bundle if discrepancies surfaced); (3) the level-appropriate **opus compose agent** (`architecture-compose-l1`, `architecture-compose-l2`, or `architecture-compose-l3`) produces the architecture document and the Card Slices. After Phase A, the command shows you the bundle, audit, and level as **transparency** — you do not approve the level, classification is deterministic. After the compose phase, you do approve the **document**; iterate if needed, then the command commits it to git and creates the workspace cards from the Card Slices. Cards do not exist before this command runs; planning agents in `/orchestrate` consume the per-card slice as the boundary truth. Plan to spend a full session on this.

The three levels in brief:

- **L1** — trivial work, 1–3 independent cards, no new contracts, no trust boundaries, no migrations, no external systems. The slicing IS the architecture; the document is slim (~50–150 lines typical) and has no separate Components or Design decisions sections.
- **L2** — coupled work that introduces internal contracts or shared verification but stays inside the project's own boundaries. The document carries a Components section and Design decisions in five-part format with per-decision verification approach inline; no formal threat model, no Context7 verification, no Clear Thought MCP ceremony.
- **L3** — substantial work: external systems, schema migrations, significant security surface, more than five new contracts, or nine+ implied cards. The document carries the full ISO/IEC 25010 quality-characteristics mapping, OWASP ASVS verification mapping when security is in scope, Context7 verification of external library claims, and structured reasoning via the eight Clear Thought MCP tools at the phases the profile maps them to.
