...
| 11 | Verification | none | Execute the test strategy from phase 8 against the implementation from phase 10. Tasks here are about running, observing, and fixing — not adding features. **Verification is not "the suite is green" — it is "the tests actually verify the code."** Apply the test-integrity discipline embodied by the `test-integrity-auditor` agent (see `agents/test-integrity-auditor.md`): run the suite and account for skipped/uncollected tests, confirm coverage of the changed production lines, trace each changed unit to a test that calls its real path and asserts on observable behavior, and reject fake tests (no-assertion, tautology, mock-the-unit-under-test, silent skip, exception-swallow, snapshot auto-bless). A wall of passing tests that verify nothing is a Phase 11 failure, not a pass — spawn that agent advisorily on the implementation (it returns findings; in a phase project there are no workspace cards to route, so fix the findings before advancing past phase 11). |
