...
A workspace of agent tooling: MCP servers, Codex plugins, skills,
hooks, system prompts, and workflows.

## Active work threads

### Codebase RAG MCP rebuild
- **Branch**: `Codex/fix-rag-tool-setup-te1aE`
- **Where**: `mcp-servers/codebase-rag/`
- **Status**: round 2 of expert review applied (commit `5a959fc`); 12/12 e2e tests passing.
- **Round 3 handoff**: `/root/.Codex/plans/codebase-rag-mcp-round-3-handoff.md`
...
- **Test command**:
  ...
  cd mcp-servers/codebase-rag/mcp-server-python && \
    ...
    python test_e2e.py
  ```
- **Out of scope** for this thread: `Codex-plugins/agentboard/`,
  `Codex-plugins/.Codex-plugin/marketplace.json`, `skills/codebase-rag-enforcer/`.
...
`/root/.Codex/plans/` holds plan files and handoffs. Read the most
