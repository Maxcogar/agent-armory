...
1. REQUIRED TOOLS: You MUST use codegraph (scan then dependencies/dependents for every relevant file), codebase-rag (rag_search), Grep, and Read to investigate BEFORE drafting your plan.
...
3. Run build + lint after changes: npm run build && npm run lint --prefix client
