...
```
mcp_codebase-rag_rag_search(query="task description", source_type="constraints")
```
...
```
mcp_codebase-rag_rag_query_impact(file_path="path/to/file.ts")
```
...
The MCP server auto-detects the project root, builds the index in a per-machine cache on first run, and runs a filesystem watcher to keep the index current as files change. There is no manual index command — just call `rag_search` or `rag_query_impact` directly.
