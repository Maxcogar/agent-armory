---
name: agentboard
description: Skill for using the AgentBoard AI project management app and its cloud-hosted MCP server. Use when managing phase-based projects (phases, milestones, documents, tasks), workspace boards (apps, boards, cards, artifacts), or authenticating the AgentBoard MCP.
---
...
### 1.2 Configure the MCP Servers

The extension ships four MCP servers in `gemini-extension.json` — no setup needed beyond installing the extension:

- `agentboard` (HTTP, cloud) — connects to `https://mcp.agent-board.app/mcp`.
- `codegraph` (stdio, Node) — launched from `${extensionPath}/../../mcp-servers/codegraph-mcp/dist/index.js`, which assumes the extension lives in the `agent-armory` monorepo at `gemini-extensions/agentboard-gemini/`.
- `codebase-rag` (stdio, Python) — launched from `${extensionPath}/../../mcp-servers/codebase-rag/mcp-server-python/server.py` in the same monorepo.
- `clear-thought` (stdio, npx) — launched via `cmd /c npx -y -p @waldzellai/clear-thought mcp-server-clear-thought`. Requires Node/npm in PATH; the package downloads on first run.

The codegraph and codebase-rag binaries must be built before first use:

```bash
cd mcp-servers/codegraph-mcp && npm install && npm run build
cd mcp-servers/codebase-rag/mcp-server-python && pip install -r requirements.txt
```

If you ever move the extension or the `mcp-servers/` siblings, update the paths in `gemini-extension.json`. If you already have either server configured in your global Gemini `settings.json`, that definition wins per §5 of the Gemini extension spec — the extension's definition is silently overridden, which is fine.

### 1.3 Authenticate

The AgentBoard MCP requires OAuth-style authentication before any tool other than the two auth tools is callable. On a fresh session — or any time tokens have expired — only `mcp_agentboard_agentboard_authenticate` and `mcp_agentboard_agentboard_complete_authentication` are visible. The full tool surface (health_check, projects, tasks, documents, workspace, etc.) appears only after the bootstrap completes.

**Bootstrap flow:**

1. **Start the OAuth flow** — call `mcp_agentboard_agentboard_authenticate` (no parameters). It returns an authorization URL.
2. **Share the URL with the user** and ask them to open it in their browser and authorize.
3. The user authorizes; their browser is redirected to a `http://localhost:<port>/callback?code=...&state=...` URL. On remote sessions the page itself may fail to load, but the URL in the browser's address bar is the value you need.
4. **Ask the user to copy the full URL from the browser's address bar.**
5. **Complete the flow** — call `mcp_agentboard_agentboard_complete_authentication` with that URL as the `callback_url` argument. The remaining tools become available immediately.

**Detecting auth state:** if you see only `mcp_agentboard_agentboard_authenticate` and `mcp_agentboard_agentboard_complete_authentication` in the agentboard tool surface, run the bootstrap. If the rest of the tools are visible, you're authenticated.
...
- Use it ONLY as the `callback_url` argument to `mcp_agentboard_agentboard_complete_authentication`.
...
After authenticating, call `mcp_agentboard_agentboard_health_check` to confirm the cloud service is reachable. If the call fails *post-auth*, the cloud service itself is unreachable — check status at `agent-board.app`.

### 1.5 Agent ID

Many tools require an `agent_id` parameter. Use your model identifier -- e.g., `"gemini-2.5-pro"`, `"gemini-2.0-flash"`. This is mandatory for traceability in the activity log.

### 1.6 Session Startup Checklist

At the start of every session, run these steps in order:

1. **Authenticate if needed** — if only `mcp_agentboard_agentboard_authenticate` and `mcp_agentboard_agentboard_complete_authentication` are visible, run the bootstrap from §1.3
2. **Health check** -- call `mcp_agentboard_agentboard_health_check` to confirm the cloud service is reachable
3. **List projects** -- call `mcp_agentboard_agentboard_list_projects` to see existing projects
4. **Get next task** -- call `mcp_agentboard_agentboard_get_next_task` with your project ID and agent ID to pick up work
...
| Tool | Purpose | Notes |
|---|---|---|
| `mcp_agentboard_agentboard_authenticate` | Start the OAuth flow. Returns an authorization URL for the user to open. | No parameters. See §1.3 for the full bootstrap. Available pre-auth. |
| `mcp_agentboard_agentboard_complete_authentication` | Complete the OAuth flow with the callback URL from the user's browser. | Required: `callback_url`. Treat the URL as a secret — see §1.3. Available pre-auth. |
| `mcp_agentboard_agentboard_health_check` | Verify the cloud service is reachable | Call first after authenticating. Not visible pre-auth. |

### 2.2 Projects

| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_projects` | List all projects | None |
| `mcp_agentboard_agentboard_get_project` | Get project details | `project_id` |
| `mcp_agentboard_agentboard_create_project` | Create a new project | `agent_id`, `name`, `project_type`, `idea`, `target_project_path?` |
| `mcp_agentboard_agentboard_advance_phase` | Move to next phase | `project_id`, `agent_id`. **Agents only call this during implementation phases (10-12).** During doc phases (2-9), the human advances the phase after approving the document. |
| `mcp_agentboard_agentboard_revert_phase` | Go back one phase | `project_id`, `agent_id`. Destructive. |
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_tasks` | List tasks (with filters) | `project_id`, `status?`, `phase?` |
| `mcp_agentboard_agentboard_get_task` | Get single task details | `task_id` |
| `mcp_agentboard_agentboard_get_next_task` | Auto-claim next actionable task | `project_id`, `agent_id`. Returns one of 4 response patterns: **(1) New task claimed** (200): a `ready` task was auto-claimed to `in-progress`, returns `{task, document}`. **(2) Existing task resumed** (200): an `in-progress` task already assigned to you, same shape. **(3) Pending review** (200): your submitted document is awaiting human review, returns `{task: null, document: null, pending_review: {...}}` -- wait. **(4) No tasks available** (404): returns `{error: "No tasks available"}`. |
| `mcp_agentboard_agentboard_create_task` | Create a new task | `project_id`, `agent_id`, `title`, and optional fields |
| `mcp_agentboard_agentboard_update_task` | Update task / transition status | `task_id`, `agent_id`, and fields to update. **Privileged tool** -- intended for Review Agents making corrections, NOT for worker agents managing their own task status. Never use on milestone tasks (`task_type: 'milestone'`) -- their status is managed automatically by the server's milestone-sync logic. |
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_documents` | List all phase documents | `project_id` |
| `mcp_agentboard_agentboard_get_document` | Get full document content | `document_id` |
| `mcp_agentboard_agentboard_submit_document` | Submit or resubmit a document for review | `document_id`, `agent_id`, `content?`, `notes` (required). **Status guard:** document must be in `template` or `rejected` status (any other returns HTTP 400). **Milestone guard:** linked milestone must be in `in-progress` status (otherwise returns HTTP 409 Conflict). **Blocks until reviewed.** |
| `mcp_agentboard_agentboard_update_document` | Correct locked documents (review agents only) | `document_id`, `agent_id`, `content?`, `status?`, `rejection_feedback?` |

**Three actors, strict separation:**

- **Worker agents** use `mcp_agentboard_agentboard_submit_document` to submit work. This locks the document and blocks the response until a human decides. Workers NEVER approve or reject.
- **Review agents** (privileged) use `mcp_agentboard_agentboard_update_document` to correct locked (submitted) documents without changing status. Review agents NEVER approve or reject -- they only fix content.
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_get_activity_log` | Get audit trail | `project_id`, `actor?`, `action?`. NOT part of the standard worker agent work loop -- primarily for debugging and project oversight. |
| `mcp_agentboard_agentboard_add_log_entry` | Record significant project-wide events | `project_id`, `agent_id`, `action`, `target?`, `detail?`. Records events NOT captured by automatic status changes. Distinct from task `notes`. NOT part of the standard worker agent work loop. |
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_apps` | List all apps | None |
| `mcp_agentboard_agentboard_create_app` | Create an app | `name`, optional `target_project_path` |
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_boards` | List boards in an app | `app_id` |
| `mcp_agentboard_agentboard_get_board` | Fetch a single board's config (`auto_transitions`, description). Use this before orchestrating to read the checkpoint settings. | `board_id` |
| `mcp_agentboard_agentboard_create_board` | Create a board | `app_id`, `name`, optional `auto_transitions` |

**Cards** are the unit of agent work. Each card moves through the orchestration pipeline column-by-column. **Cards do NOT use the phase task state machine** — their lifecycle is governed entirely by board column transitions and the orchestration pipeline. Never reach for `mcp_agentboard_agentboard_update_task` on a card.

| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_list_workspace_cards` | List cards on a board (filter by status/column) | `board_id`, `status?` |
| `mcp_agentboard_agentboard_get_card` | Fetch a card by ID. Returns artifacts bundled. | `card_id`, `response_format?` |
| `mcp_agentboard_agentboard_get_next_card` | Auto-claim the next actionable card from a column. | `board_id`, `agent_id`, `column?` |
| `mcp_agentboard_agentboard_create_workspace_card` | Create a card on a board. Use when an agent needs to spawn follow-up work from the card it's currently on (e.g. a planning agent splits a card into smaller implementation cards). Don't pollute one card with multiple distinct units of work. | `board_id`, `title`, `description?`, `priority?`, `depends_on?` |
| `mcp_agentboard_agentboard_update_workspace_card` | Update card fields (title, description, notes, status/column). | `card_id`, `agent_id`, fields to update |
...
| Tool | Purpose | Key Parameters |
|---|---|---|
| `mcp_agentboard_agentboard_submit_workspace_artifact` | Append an artifact to a card. Triggers the SUBMISSION QUALITY GATE — the artifact must be free of TODO/TBD/placeholder text and reference specific files/line numbers. | `card_id`, `agent_id`, `artifact_type`, `content` |
| `mcp_agentboard_agentboard_list_workspace_artifacts` | List artifacts on a card without pulling the full card. Use this for cross-card lookups (e.g. an implementation agent reading the planning card's plan artifact). | `card_id` |
| `mcp_agentboard_agentboard_get_workspace_artifact` | Fetch a single artifact by ID. Cheaper than `get_card` when only one artifact body is needed. | `artifact_id` |
...
**Advance rule:** During doc phases (2-9), the human advances the phase after approving the document -- agents do not call `advance_phase` here. During implementation phases (10-12), agents call `mcp_agentboard_agentboard_advance_phase` to advance freely.

### 3.2 Milestone Workflow (Phases 1-8)

Milestone task status is a **reflection** of document status, managed automatically by the server's milestone-sync logic. Agents should NEVER use `mcp_agentboard_agentboard_update_task` on milestone tasks. The agent's only direct interaction is submitting the document via `submit_document` -- all milestone state transitions flow automatically from that.

Milestone tasks are auto-created for phases 1-8 and linked to phase documents. The workflow:

1. **Claim milestone** -- `mcp_agentboard_agentboard_get_next_task` auto-claims the next `ready` milestone
...
**Phase 9 exception:** Phase 9 (Task Breakdown) has a document but no milestone task. Submit the document manually via `mcp_agentboard_agentboard_submit_document`. The human advances the phase after approval.
...
```
1. mcp_agentboard_agentboard_health_check          -- verify cloud service is reachable
2. mcp_agentboard_agentboard_list_projects         -- find or create project
3. mcp_agentboard_agentboard_get_next_task         -- claim work
4. (do the work: read template, fill document, or implement code)
5. mcp_agentboard_agentboard_submit_document       -- for milestones: submit and wait for review
   OR submit for review with notes  -- for implementation: see Section 4.3
6. mcp_agentboard_agentboard_get_next_task         -- claim next work item
7. (repeat 4-6 until phase complete)
8. (human advances phase during doc phases 2-9)
   mcp_agentboard_agentboard_advance_phase         -- agents call this ONLY during implementation phases 10-12
...
```
mcp_agentboard_agentboard_create_project(
  agent_id="my-agent",
...
mcp_agentboard_agentboard_get_next_task(project_id="<id>", agent_id="my-agent")
...
mcp_agentboard_agentboard_update_task(
  task_id="<id>",
  agent_id="my-agent",
  status="review",
...
mcp_agentboard_agentboard_update_task(task_id="<id>", agent_id="my-agent", status="blocked")

# Unblock (must return to previous_status)
# First check what previous_status is:
mcp_agentboard_agentboard_get_task(task_id="<id>")
# Then transition back:
mcp_agentboard_agentboard_update_task(task_id="<id>", agent_id="my-agent", status="in-progress")
```
...
| Tool | Purpose |
|---|---|
| `mcp_codegraph_codegraph_scan` | Scan project, build dependency graph. **Must call first.** |
| `mcp_codegraph_codegraph_get_stats` | File counts, most connected/depended-on files |
| `mcp_codegraph_codegraph_get_dependencies` | What does file X import? |
| `mcp_codegraph_codegraph_get_dependents` | What imports file X? (what breaks if X changes) |
| `mcp_codegraph_codegraph_get_change_impact` | Blast radius of changing file(s) |
| `mcp_codegraph_codegraph_get_subgraph` | Dependency subgraph around a file |
| `mcp_codegraph_codegraph_find_entry_points` | Find entry point files |
| `mcp_codegraph_codegraph_list_files` | List all scanned files |

**When to use codegraph in the workflow:**

| Phase | Template Sections It Feeds | What To Do |
|---|---|---|
| **2 -- Codebase Survey** | §2 Directory Structure, §3 Entry Points, §4 Module Inventory, §8 Dependency Analysis, §13 Integration Points | Run `mcp_codegraph_codegraph_scan` on the target project first. Use `get_stats` for the overview, `find_entry_points` for §3, `list_files` + `get_dependencies` for §4, `get_stats` for §8. This is your primary research tool for this phase. |
...
The codebase-rag MCP exposes two tools. The server auto-detects the project root, auto-builds the index in a per-machine cache on first call, and runs a filesystem watcher to keep it current. There is no setup, init, status, or health-check tool — just call the search tools directly.

| Tool | Purpose |
|---|---|
| `mcp_codebase-rag_rag_search` | Semantic search across the project. Inputs: `query`, `num_results` (default 5), `source_type` (`"all"` (default) / `"docs"` (constraints + patterns) / `"code"` / `"constraints"`). Returns ranked results with file paths and relevance scores. |
| `mcp_codebase-rag_rag_query_impact` | Show what depends on a file before you change it. |
...
```
1. mcp_codegraph_codegraph_scan(path="<target>")     -- REQUIRED every session (graph is in-memory only, lost when MCP server restarts)
2. mcp_codebase-rag_rag_search("<first query>")         -- RAG auto-bootstraps; first call in a never-indexed project takes a few seconds
```
