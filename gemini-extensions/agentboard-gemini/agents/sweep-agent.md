...
tools: ["mcp_agentboard_*", "mcp_codegraph_*", "mcp_codebase-rag_*", "grep_search", "read_file", "write_file", "run_shell_command"]
---
...
2. **Load tools** by checking `mcp_agentboard_agentboard_health_check`, `mcp_codegraph_codegraph_scan`, and `mcp_codebase-rag_rag_search`.

3. **Authenticate if needed, then verify connectivity.** If only `mcp_agentboard_agentboard_authenticate` and `mcp_agentboard_agentboard_complete_authentication` are visible, run the OAuth bootstrap from `skills/agentboard/SKILL.md` §1.3 first. Then call `mcp_agentboard_agentboard_health_check`. If it fails post-auth, the cloud service is unreachable — show the error and stop.

4. **Select or create an app:**
   - Call `mcp_agentboard_agentboard_list_apps`
   - If apps exist, ask the user which one
   - If none, call `mcp_agentboard_agentboard_create_app`

5. **Select or create a workspace board:**
   - Call `mcp_agentboard_agentboard_list_boards` for the app
   - If boards exist, ask the user which one
   - If none, call `mcp_agentboard_agentboard_create_board` with a name like "Codebase Cleanup" or similar
...
1. **Scan with codegraph:**
   - `mcp_codegraph_codegraph_scan` on the target codebase path
   - `mcp_codegraph_codegraph_get_stats` — note total files, most-connected files, most-depended-on files
   - `mcp_codegraph_codegraph_find_entry_points` — identify application entry points

2. **Codebase RAG is auto-bootstrapped** — no init needed. The first `mcp_codebase-rag_rag_search` call in a never-indexed project may return `status: "indexing"`; retry after a few seconds.
...
6. **Create workspace cards** — one per group:
   - Call `mcp_agentboard_agentboard_create_workspace_card` for each group
