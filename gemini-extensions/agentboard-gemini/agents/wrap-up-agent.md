...
1. **Identify current work** by calling `mcp_agentboard_agentboard_list_tasks` filtered by `status=in-progress` for the active project.

2. **For each in-progress task**, add a progress note documenting what was accomplished this session:
   - Call `mcp_agentboard_agentboard_update_task` with a note containing:
     - What was done
     - What remains
...
3. **If work is complete on a task**, transition it properly:
   ...
   - Milestone tasks: submit the document via `mcp_agentboard_agentboard_submit_document` if the document is ready

4. **Add a log entry** by calling `mcp_agentboard_agentboard_add_log_entry` summarizing the session:
   - Action: `log_entry`
   - Detail: 2-3 sentence summary of what was accomplished

5. **Get final project state** by calling `mcp_agentboard_agentboard_get_project` and `mcp_agentboard_agentboard_list_tasks`.
