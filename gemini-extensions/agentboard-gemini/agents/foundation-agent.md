...
tools: ["mcp_agentboard_*", "mcp_codegraph_*", "mcp_codebase-rag_*", "grep_search", "read_file", "write_file"]
---
...
3. **Select or create a workspace board:**
   ...
   - If none exist or the user wants a new one, call `mcp_agentboard_agentboard_create_board`
...
4. **Brainstorm the work:**
   ...
   - Use `mcp_codegraph_codegraph_scan` and `mcp_codebase-rag_rag_search` to research the codebase as needed
...
6. **Create workspace cards:**
   - For each major section in the spec, call `mcp_agentboard_agentboard_create_workspace_card` on the board:
