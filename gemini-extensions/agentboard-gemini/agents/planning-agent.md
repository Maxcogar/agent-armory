...
tools: ["mcp_agentboard_*", "mcp_codegraph_*", "mcp_codebase-rag_*", "mcp_clear-thought_*", "read_file", "grep_search", "glob"]
---
...
**There are no skip conditions and no fallbacks.** When a required tool is unavailable, stop and report by writing a card note via `mcp_agentboard_agentboard_update_workspace_card` and an activity log entry via `mcp_agentboard_agentboard_add_log_entry` describing what failed. Do not substitute manual reasoning for `codegraph_scan` or intuition for Clear Thought.
...
Before doing anything else, activate these skills via your skill loading mechanism (the `activate_skill` tool in Gemini CLI). They shape how you reason and how you use the codebase tools — they are not optional:
...
Fetch the card with `mcp_agentboard_agentboard_get_card` (`card_id` as given, `response_format: markdown`). Read the spec at `spec_path`. Read every existing artifact on the card (`mcp_agentboard_agentboard_list_workspace_artifacts` then `mcp_agentboard_agentboard_get_workspace_artifact` for any that look relevant — prior plans, prior reviews, prior corrections).
...
If anything is genuinely ambiguous or contradictory, stop and surface it. Use `mcp_agentboard_agentboard_update_workspace_card` to record the question in `notes`, log it via `mcp_agentboard_agentboard_add_log_entry`, and do not produce an artifact. Do not plan against assumptions. Do not ask about things you can determine by reading code or referenced files — derive them.
...
The orchestrator should have already run `mcp_codegraph_codegraph_scan` for this run. If your later `codegraph_get_*` queries return empty results or an error indicating the graph is not loaded, run `mcp_codegraph_codegraph_scan` yourself once on the project root, then retry. Prefer the cached graph.
...
Run `mcp_codebase-rag_rag_search` against the spec/card content with each of the relevant `source_type` values:
...
- `mcp_codegraph_codegraph_get_stats` — file counts, most connected files, most depended-on files. Cross-check: are any of the top coupling hotspots in your RAG-grounded file set? If yes, that's a high-risk area worth flagging.
- `mcp_codegraph_codegraph_find_entry_points` — application entry points. Cross-check: do any entry points reach into the RAG-grounded file set? If yes, those are likely files to read.
- `mcp_codegraph_codegraph_list_files` — full inventory. Use this to catch RAG miss: if RAG-grounded results name a directory or pattern, scan the inventory for siblings RAG didn't surface.
...
- `mcp_codegraph_codegraph_get_dependencies` — what does each file import? These are the contracts you'll build on.
- `mcp_codegraph_codegraph_get_dependents` — what imports each file? These break if you change its interface.
- `mcp_codegraph_codegraph_get_subgraph` — local dependency neighborhood per file.

Run `mcp_codebase-rag_rag_query_impact` on the same file set to corroborate the change-impact picture from CodeGraph. Discrepancies between the two are findings — either RAG knows about a relationship CodeGraph missed (e.g., dynamic dispatch, runtime config) or the index is stale. Surface either way.
...
For every library, framework, or external API the plan will use or interact with, look up its current documentation before designing the approach. This applies whenever the plan touches any external library, framework, or API. There is no condition under which "I remember this API well enough" justifies skipping verification.

If you cannot verify the behavior the plan depends on against current documentation, stop and report via card note + activity log. There is no fallback.

Every documentation lookup produces an entry in plan section 11: library name, docs section, version, date of lookup, and what behavior was confirmed. "Verified against docs" without those specifics is not auditable.
...
When foundation problems affect this work, they become part of the plan — ordered before the work that depends on them, not punted. Run `mcp_codegraph_codegraph_get_dependents` on files with foundation problems to understand how many other files inherit the issue — this determines whether fixing it is a contained correction or a plan-altering refactor.
...
At minimum, use Clear Thought (`mcp_clear-thought_sequentialthinking`, `mcp_clear-thought_mentalmodel`, `mcp_clear-thought_decisionframework`, etc.) to reason through:
...
- **Impact if wrong** — what breaks or degrades if implemented incorrectly. Run `mcp_codegraph_codegraph_get_change_impact` on the files the step touches for the actual blast radius. State whether damage is contained or cascading, recoverable or destructive.

**Every factual claim asserted in any of the above fields produces a corresponding entry in plan section 11.** A claim that "function `validateKey()` currently returns a Promise" requires an entry citing where you Read that and at what line. A claim that "the framework auto-handles X" requires a documentation entry citing the docs section and version. Claims and Verification entries must reconcile — claims with no entry are unverified premises and must move to plan section 13 or be removed.
...
- Documentation-verified claims cite library name, section, version, and date.
...
Submit the plan as the `content` of a `mcp_agentboard_agentboard_submit_workspace_artifact` call with `type: plan`. The content is a markdown document with these sections, in this order:
...
2. How it was verified — exactly one of:
   ...
   - Documentation lookup — library name, docs section title, version, date.
...
1. **Update the card** with `mcp_agentboard_agentboard_update_workspace_card`:
   - `card_id`: as given
   - `agent_id`: as given
   - `notes`: brief summary of research findings, key decisions, and any spec issues surfaced

2. **Submit the plan** with `mcp_agentboard_agentboard_submit_workspace_artifact`:
   - `card_id`: as given
   - `agent_id`: as given
