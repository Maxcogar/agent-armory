...
## expert-dev-tools defects

Defects in the expert-dev-tools plugin go in
`claude-plugins/expert-dev-tools/docs/defects/` — one file per defect, with an index
generated by `scripts/derive-defect-index.mjs`. Never patch the plugin to get past a
problem, and never write plugin issues into whatever project happened to hit them; a
report outside that directory is stranded and nothing will act on it. Corrections leave
the queue through the plugin's own process — plan, review rounds, implementation, version
bump — not by hand-editing the workflow.
