...
plans). Preview builds gated it behind `CLAUDE_CODE_WORKFLOWS=1`; that env var is
no longer needed. The tool comes with the `/workflows` slash command — a live tree of
...
| `args` | any | Optional input exposed to the script as the global `args`. In the current runtime, JSON values arrive live (pass real JSON, not a JSON-encoded string). Preview builds serialized this field to a string. See the `args` global below. |
...
In the **current runtime**, JSON `args` pass through as live values:
`Workflow({ args: { minUsers: 5 } })` gives the script a real object and
`args.minUsers` works; arrays support `args.map(...)`. A plain string stays a
string; nothing passed → `undefined`. (Preview builds — probed 2026-05-29 —
serialized `args` to a JSON string first; `args.minUsers` was `undefined` until
you parsed it.)

A script that takes input should still **normalize once at the top** — parse when
it is a string, pass a live value through — so it behaves under either runtime
and when a caller mistakenly passes a JSON-encoded string:
