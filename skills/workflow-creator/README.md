...
plans) — no setup needed.

1. Ask Claude to build one, for example: "create a workflow that reviews my
   branch across bugs, security, and tests, then verifies each finding."

2. Claude uses this skill to write the file, validate it, and run it. Watch live
...
Claude Code binary, not guessed. This copy was imported into Maxcogar/agent-armory
on 2026-07-20 from ray-amjad/claude-code-workflow-creator and updated for the
released Workflow tool (env-var gating removed; `args` now passes live JSON). If something stops matching once the feature is
