...
4. If verification fails: diagnose the root cause and fix it **within the step's authorized scope**, then re-verify. If the failure reveals one of the five stop categories, halt per Step 4. If it is just a bug in your own implementation of the step, fix and re-verify.
...
Stopping is reserved for cases where continuing would either violate a non-negotiable rule or build work on a false premise, or build a flaw the plan carries. Five categories qualify, and only these five:
...
When one of the five categories triggers and your evidence reproduces, **emit a STOP REPORT in this exact format and halt:**
