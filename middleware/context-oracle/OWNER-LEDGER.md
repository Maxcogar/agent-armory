...
| OL-P2 | **Reuse-first over from-scratch.** Your verbatim words: *"I'd rather use existing stuff if it exists than to remake it from scratch because it will not be made nearly as well as something that already exists."* Applies to the build: prefer mature existing libraries/tools (e.g. tree-sitter for parsing/indexing, code-maat/PyDriller for co-change mining, SQLite/FTS5 for the store, gitleaks/trufflehog/detect-secrets for secret redaction) over bespoke reimplementation; build only the novel decision engine. (This is also independently sound engineering — DRY / don't-reinvent — so the spec can carry it as a constraint on that basis even before you confirm; this entry records that it is also **your stated preference**.) | Max, in chat, 2026-08-25 | ⏳ Awaiting your "confirmed" / correction |
