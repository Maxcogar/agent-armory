...
| OL-R5 | The answer-drift trigger scoped to **"writing code,"** with the **"agent silently ends its turn" case described as out of scope** (the former pending OL-P3 framing). | **Agent's narrow proxy + negative-space padding, not Max's rule.** "Writing code" was one agent's proxy; the real trigger is *any next move that is not a direct answer or an action to provide an answer* (OL-C5). Max 2026-08-25: *"that is a terrible way to put it… stop leaving in shit that it isnt… if you have to describe what it isnt, then the description of what it is isnt defined well enough."* Superseded by **OL-C5**; define the block positively, never by exclusion. | Max, 2026-08-25 |
...
## ⏳ PENDING — attributed to Max Cogar in project documents, awaiting his word

| ID | The attribution as found | Where it came from | What Max decides |
|---|---|---|---|
| OL-P1 | A review-round convergence rule: *"two consecutive rounds where the corrections themselves introduced defects means stop and diagnose the correction process externally; five rounds without convergence means stop."* | Written into `docs/STATUS.md` at commit `21221c5` (2026-09-07) by an agent session as "Convergence rule from the owner" and "Max Cogar's rule is unchanged", then cited by later sessions as "the owner's rule" (`docs/collapse-log.md`, 2026-09-07 entry) and "the owner's five-round cap" (same entry; `docs/STATUS.md`). No project document holds Max Cogar's own words or the message they came from, and no CONFIRMED row backs it — found 2026-09-11 while re-reading the record. The 2026-09-07 sessions did act on it (the manual review loop stopped at round 5). | Confirm it (it becomes a CONFIRMED row in his words) or reject it (it moves to REJECTED and the two citations are reworded to say the sessions stopped on an unconfirmed rule). Until then it is not authority: no session stops or continues a review round because of it. |
...
entries anywhere in the project, 2026-09-08). One entry is PENDING (OL-P1, the review-round cap).
