...
**What Phase 0 is for (added 2026-08-01).** Phase 0 exists to **produce the
answers Phases 1 and 2 cannot be designed without.** Their exits are
measurements — Phase 1's is *"measured silence and hit rates reviewed against the
...
such a measurement to be trustworthy. Applied, this obliges Phase 0 to span:

- **Provenance classes** — `self_serve_cost` (architecture D10) is keyed to the
  provenance class of the fact, so a bar exercised on one class calibrates only
  that class. At minimum: a history-derived fact, a structure-derived fact, and a
  mapping-derived fact.
- **Trigger channels** — latency, delivery and false-fire behaviour are
  per-channel. A channel Phase 0 never speaks on yields no data about that
  channel, and Phase 1 is then designed against nothing for it.
- **Delivery costs** — the free-versus-turn-spending distinction (FR-O4a,
  `[OWNER-12]`) is the one cost the owner has accepted, so its real rate must be
  observed, which requires a live stop-class delivery.

**Why the second exit clause is load-bearing, not a usability nicety.** Phase 0's
exit includes *"the owner runs it on a real project without incident."* The
measurements are only valid if generated by real use: a Phase 0 the owner would
not choose to run produces numbers that describe nothing, and Phases 1 and 2
would then be tuned against an artefact of the rig. Utility is therefore a
requirement of the measurement purpose, not a competing goal.

**What this rules out.** Deciding Phase 0's contents by build cost, by which
genres are easiest to make work, or by which need no model. Those are not the
question. A genre whose exclusion costs a measurement stays in even if it is the
most expensive thing in the phase; a mechanism that produces no measurement and
gates nothing is not Phase 0 work regardless of how cheap it is.
