...
## 2026-08-25 — Fake deferrals purged from the spec. Max flagged that `/expert-spec` forbids the "resolve-later" deferrals agents keep planting, and the spec's §13 was full of them. Removed the three self-narration subsections ("Architecture-owned (mechanism deferred)", "Measured on the exit run", "Gate-B re-confirm before build") — those enumerate architect-owned mechanisms and tunable defaults the requirements already carry inline, which is exactly the forbidden move of naming a decision only to disclaim it. §13 now lists only the two genuine external unknowns the spec cannot close by decision: unverified `[HOOKS]` specifics (event names/timeouts, treated as assumed) and subagent-`additionalContext` parent propagation; plus the thin-history known limit. In §9 the `re-confirm at build (§13)` hedges on every "prior pass" citation became a plain recency fact ("prior pass" = last verified in an earlier review, not this session), and the "Re-confirm at build" process-narration paragraph was cut. Next: the independent review of the fully-revised spec (blocking model + this cleanup have not been attacked yet).
