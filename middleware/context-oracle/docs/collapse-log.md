...
## 2026-09-07 — the review loop itself went hollow: "keep dispatching rounds until one comes back clean" has no phase-goal check, and became mechanism-not-mission on its own terms

This session ran an iterative collapse-hunt + expert-review loop against
`docs/plans/plan-phase-a.md`, with one stated exit condition: dispatch
another round until a round returns completely clean. By round 12, that
loop had spent four consecutive rounds (9-12) narrowing successive races
in one mechanism, Step 37's reindex lock — each round's fix correctly
closing the exact scenario the prior round found, each in turn found to
have a new, narrower race in its own remediation code. Round 12 replaced
the mechanism structurally rather than patching a fifth time. A round-13
pass was then dispatched anyway, to adversarially re-verify round 12's
fix, with no different reasoning than rounds 9 through 12 used to justify
themselves — the loop's only stated stopping condition ("a round comes
back clean") does not distinguish "still finding real defects" from
"reviewing a design document's prose past the point where further review
adds anything execution against real code wouldn't settle faster and more
concretely."

**Why this is a collapse, not just a long day.** `CLAUDE.md` dominating
rule 3 says outright: "passing review is not the goal... a mechanism
built to survive review rather than serve the goal drifts further from
it each round. When a non-trivial mechanism only passes review without
serving the phase goal, cut it: that is a finding, not a decision to
defend." This project already has a named precedent for exactly this
shape — `AD-9`, "elaborated over ten rounds to look like it works,"
abandoned 2026-09-04 as "the exact slop that cost three prior versions of
this tool." The review loop run this session was never checked against
that rule. It was designed and operated purely procedurally: dispatch,
apply findings, repeat, with the round count itself treated as evidence
of rigor rather than as a cost to weigh against the phase goal. Twelve
(then a thirteenth, interrupted) rounds against one planning document,
four of them consumed by one mechanism whose actual correctness will be
settled by `T37-3` running against real code once Steps 3/21/37 exist,
is the same shape as `AD-9`: a mechanism (here, the review process itself)
that kept "passing its own next round" without ever being asked whether
continuing served Phase A's actual goal (an honest foundation, built and
measured on real repos) or was just accumulating review cycles on a
document.

**The sharpened lesson.** An iterative verification loop needs a
phase-goal check as an exit condition, not only a correctness check. "No
new findings" and "further review would cost more than it's worth, given
what execution against real code will settle anyway" are different
questions, and a loop built to answer only the first one will run past
the second one indefinitely, especially on a document (a plan) whose
claims are provable by construction (documented engine semantics reused
from an already-trusted site in the same document) rather than by ever
more adversarial reading. Before dispatching another round of review —
of anything, not only this mechanism — ask rule 3's own question first:
does another round serve the phase goal more than moving to the next
lifecycle stage would? If the honest answer is "the remaining risk is
better and more cheaply settled by building and running the thing," stop
dispatching rounds and go build it.

**Class: mechanism-not-mission**, at the process layer this time: the
review loop's own design (dispatch until clean) was defended by how it
worked (a convergence loop, modeled on the architecture document's own
nine-round history) rather than by whether continuing it, round after
round, still served the phase goal — the identical failure shape this
class already names for individual decisions, recurring here in the
process built to catch that failure shape in decisions.
