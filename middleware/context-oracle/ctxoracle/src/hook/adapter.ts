...
export function toInternalEvent(j: HookJson, kindFromArgv: EventKind): InternalEvent & { agentId?: string } {
  const cwd = str(j.cwd) ?? process.cwd();
  const input = (j.tool_input ?? {}) as Record<string, unknown>;
  const rawTarget = str(input.file_path) ?? str(input.notebook_path) ?? str(input.path);
  const targetPath = rawTarget === undefined ? undefined : path.isAbsolute(rawTarget) ? path.relative(cwd, rawTarget) : rawTarget;
  ...
  return {
    ...
    consumer: agentId === undefined ? 'main' : 'subagent',
    agentId,
