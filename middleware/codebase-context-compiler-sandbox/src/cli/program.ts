/**
 * Commander wiring for the ctxpack CLI.
 *
 * This module (and its transitive imports) pull in `node:sqlite`. It is loaded
 * *dynamically* by `main.ts` so that the experimental-warning filter installs
 * before the `node:sqlite` builtin is translated by the ESM loader — see
 * `main.ts` and `suppress-experimental-warnings.ts`.
 */
